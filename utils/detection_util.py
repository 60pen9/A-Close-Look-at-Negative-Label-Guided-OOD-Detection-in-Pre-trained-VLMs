    ...
    with torch.no_grad():
        for batch_idx, (images, labels) in enumerate(tqdm_object):
            ...
            if args.model == 'CLIP':
                ...
                global_features = model.encode_image(images).float()
                ...
                if args.mc_local_number == 0:
                    ...
                    # score = []
                    #
                    # sim_pos = global_features @ text_features_pos.T
                    #
                    # for j in range(args.ngroup):
                    #
                    #     if args.ngroup > 1:
                    ...
                    #     text_features_neg_j = text_features_neg[j]
                    #
                    #     sim_neg_j = global_features @ text_features_neg_j.T
                    #
                    #     # sim_pos_neg_j = sim_neg_j
                    #     sim_pos_neg_j = torch.cat((sim_pos, sim_neg_j), dim=1)
                    #
                    ...
                    #         grad = 1 - eta * (g ** (1.0 / r_star - 1.0)) * term
                    #         beta = beta - lr * grad
                    #
                    #
                    #     output_pos = torch.exp((sim_pos - beta) / temp)
                    #
                    #     output_pos_neg_j = relu((sim_pos_neg_j - beta) / temp).pow(r_star).mean(dim=1,
                    #                                                                             keepdim=True).pow(
                    #         1.0 / r_star)
                    #
                    #     Z_j = torch.exp(output_pos_neg_j * eta)
                    #
