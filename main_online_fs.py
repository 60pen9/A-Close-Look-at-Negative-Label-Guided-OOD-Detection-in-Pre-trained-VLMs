...
# hyper-parameters
parser.add_argument('--temp', default=0.01, type=float) # 0.012
...
parser.add_argument('--eta', default=0.9, type=float) # 0.9

parser.add_argument('--r', default=1.03,type=float) # 1.03

parser.add_argument('--beta', default=-0.14, type=float) #-0.14

parser.add_argument('--epochs', default=20, type=int) #20

parser.add_argument('--lr', default=1e-2, type=float) #1e-2
...
def main():
    ...
    # image_feature_dict = torch.load('./CLIP_features.pth')

    image_feature_dict = torch.load('./CLIP_features_B16.pth')
    image_features_in = image_feature_dict["ImageNet"]
    image_feature_dict = torch.load('./CLIP_features_B16_V2.pth')
    image_features_in_V2 = image_feature_dict["ImageNet-V2"]
    image_feature_dict = torch.load('./CLIP_features_B16_S.pth')
    image_features_in_S = image_feature_dict["ImageNet-S"]
    image_feature_dict = torch.load('./CLIP_features_B16_C.pth')
    image_features_in_C = image_feature_dict["ImageNet-C"]

    image_features_in = torch.cat((image_features_in, image_features_in_V2, image_features_in_S, image_features_in_C), dim=0).cuda().double()
    print(image_features_in.size())

    # text_feature_dict = torch.load('./selected_neg_labels/neg_dump.pth')
    # text_feature_dict = torch.load('./selected_neg_labels/neg_dump_new.pth')  # neglabel B16
    # text_feature_dict = torch.load('./neg_dump_new.pth')
    ...
    # text_feature_dict = torch.load('./neg_dump_new_B16_common.pth')
    # text_feature_dict = torch.load('./neg_dump_new_B16_POS.pth')


    print('load pre-trained CLIP text features')
    text_features_pos = text_feature_dict['pos_emb'].cuda().double()
    id_class_num = text_features_pos.shape[0]

    text_features_neg = text_feature_dict['neg_emb_selected'].cuda().double()
    ...
    #
    # kmeans = faiss.Kmeans(text_features_neg.shape[1], args.ngroups, niter=100, verbose=True, gpu=False, seed=args.seed)
    # kmeans.train(to_np(text_features_neg))
    # D, I = kmeans.index.search(to_np(text_features_neg), 1)
    # args.I = np.squeeze(I)
    # print(np.unique(I, return_counts=True)[1])
    ...
    for out_dataset in out_datasets:
        ...
        image_features_out = image_feature_dict[out_dataset].cuda().double()
        ...
        sim_pos = image_features @ text_features_pos.T
        scores = torch.softmax(sim_pos / args.temp, dim=1)
        scores = to_np(scores)
