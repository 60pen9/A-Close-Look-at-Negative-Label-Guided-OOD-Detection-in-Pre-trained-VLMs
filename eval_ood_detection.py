...
def process_args():
    ...
    parser.add_argument('--in_dataset', default='ImageNet-R', type=str,
                        choices=['ImageNet', 'ImageNet100', 'bird200', 'car196', 'food101', 'pet37','ImageNet-A','ImageNet-R','ImageNet-S','ImageNet-V2', 'ImageNet-C'],
...
def main():
    ...
    elif args.in_dataset in ['ImageNet', 'ImageNet100', 'bird200', 'car196', 'food101', 'pet37','ImageNet-A','ImageNet-R','ImageNet-S','ImageNet-V2','ImageNet-C']:
    ...
    with (torch.no_grad()):
        ...
        print(text_features_neg_noun.shape)
        print(text_features_neg_adj.shape)
    ...
    text_features_pos /= text_features_pos.norm(dim=-1, keepdim=True)
    text_features_pos = text_features_pos.to(torch.float32)
    ...
    for i in range(0, noun_length+adj_length, emb_batchsize):
        ...
        sim = torch.quantile(tmp, q=0.95, dim=-1)
    ...
    print_measures(log, np.mean(auroc_list), np.mean(aupr_list),
                   np.mean(fpr_list), method_name='ours')
    ...
    # torch.save(feature_dict, './CLIP_features_B16_C.pth')
